...
def main():
    ...
    print("\n[+] Leyendo base de datos de 164 Parques de Aventura...")
    try:
    ...
    for idx, parque in enumerate(parques[:3]):
        ...
        tema_objetivo = f"Guía Técnica y de Seguridad: {nombre_parque} ({categoria})"
        
        print(f"\n[{idx+1}/{len(parques)}] Investigando: {nombre_parque} en {region_objetivo}...")
        
        contexto_real = extraer_fqsa_region(qclient, region_objetivo, tema_objetivo)
        
        if not contexto_real.strip():
            print(f"  [!] No se encontró info vectorial para {region_objetivo}. Saltando.")
            continue
            
        print(f"  [+] Redactando artículo SEO...")
        ...
        # Guardar en disco
        filename = f"seo_{nombre_parque.lower().replace(' ', '_').replace('(', '').replace(')', '').replace('á', 'a')}.md"
        ruta_salida = OUTPUT_DIR / filename
        
        with open(ruta_salida, 'w', encoding='utf-8') as f:
            f.write(articulo_md.strip())
            
        print(f"  ✅ Artículo guardado en: {ruta_salida}")
